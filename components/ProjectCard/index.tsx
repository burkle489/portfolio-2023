    ...
    <div className="bg-white absolute top-1/4 h-1/2 w-full border-2 border-black py-8 px-10 pt-20">
      <div>
        ...
        <div className="flex gap-4 absolute top-8 right-10">
          ...
          <SocialLink social={SOCIALS.WEB} />
        </div>
        <div className="flex justify-start items-end">
          <div className={cx("flex flex-wrap gap-2 text-sm italic")}>
            {tools?.map((item) => (
        ...
        {screenshot && (
          <Image
            ...
            className="border-2 border-black relative mt-8"
          />
        )}
        {logo && (
          <Image
            src={logo}
            alt="pokeball"
            className="absolute -bottom-20 -right-20 z-10 h-52 w-52"
          />
        )}
      </div>
    </div>
